...
//	double scaleSatUpper = 0.78;
	Scalar SkinBoundLow = Scalar(0, scaleSatLower*255, 0);
	Scalar SkinBoundHigh = Scalar(25, scaleSatUpper*255, 255);

	cvtColor(a_tImgIn, tHSVImg, CV_RGB2HSV);
...
//		a_CBG_Model(tSkinImg, tSkinImg, 0.005);
	}
	else
	{
		a_CBG_Model(a_tImgIn, tFG_Mask, 0);
//		a_CBG_Model(tSkinImg, tSkinImg, 0);
	}
#endif

	tForeground = tSkinImg.clone(); // for test
//	tForeground = tFG_Mask.clone();
...
//	double scaleSatUpper = 0.78;
	...
	if (a_bUseBackground) {
		...
		if (a_bBG_Update)
		{
			...
			LOGE("update model:%f", a_eLearnRate);
		}
		else
		{
...
//	tForeground = ForegroundDetect(tSrcBuffer, true, g_BG_Model);
	tForeground = ForegroundDetect2(tSrcBuffer, a_bUseBackground, a_bLearn, a_eLearnRate, g_BG_Model_2);
...
Mat FindPolygonAndContour(Mat a_tSrc, const GestureReconition_Cfg a_tCfg) {
	...
	for (int i = 0; i < lLocalSize; i++)
	{
		circle(tContour, Point(hull[lMaxContoursId][i]), 4, Scalar(0, 155, 100), 2);
	}

//	vector<Point> vtMergeHull = MergeHull(hull[lMaxContoursId]);
//	lLocalSize = vtMergeHull.size();
//	for (int i = 0; i < lLocalSize; i++)
//	{
//		circle(tContour, Point(vtMergeHull[i]), 4, Scalar(0, 155, 100), 2);
//	}
...
Mat DrawResult(Mat a_tSrc, const GestureReconition_Cfg a_tCfg)
{
	...
	if (lMaxContoursId >= hull.size()) {
		// 找不到特徵點，直接返回 (C.C.C.)
		char strFrame[256];
		sprintf(strFrame, "Finger Num = 0");
		putText(tFinger, strFrame, cvPoint(20, 20), 1, 1, CV_RGB(255, 255, 0), 2, 16);
		return tFinger.clone();
	}
	...
	// get convexity Defects Local
	convexityDefects(Mat(contours[lMaxContoursId]), vtMergeHullId, DefectsLocal);
//	convexityDefects(Mat(contours[lMaxContoursId]), hullsI[lMaxContoursId], DefectsLocal);
...
//		}
	...
	char strFrame[256];
	sprintf(strFrame, "Finger Num = %d", GetFingerNum(vtNodeList));
	putText(tFinger, strFrame, cvPoint(20, 20), 1, 1, CV_RGB(255, 255, 0), 2, 16);
